...
# ======================================================
# کلاس دیتابیس اصلی با مدیریت قفل
# ======================================================
class MainDatabase:
    ...
    def _get_connection(self, thread_id=None):
    ...
    def _close_connection(self, thread_id=None):
        if thread_id is None:
            thread_id = threading.get_ident()
        with self._lock:
    ...
    def add_user(self, user_id, full_name, username):
        self._execute_update('INSERT OR REPLACE INTO users (user_id, full_name, username, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP)', (user_id, full_name, username))

    def get_user(self, user_id):
        rows = self._execute_query('SELECT * FROM users WHERE user_id = ?', (user_id,))
        if rows:
            return dict(rows[0])
        return None
    ...
    def update_user(self, user_id, **kwargs):
        ...
        self._execute_update(f'UPDATE users SET {set_clause}, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?', values)

    def get_pending_requests(self):
        rows = self._execute_query('SELECT * FROM users WHERE request_sent = 1 AND admin_approved = 0 AND rejected = 0 AND step IS NULL ORDER BY request_date DESC')
        return [dict(row) for row in rows]

    def get_pending_login(self):
        rows = self._execute_query('SELECT * FROM users WHERE admin_approved = 1 AND self_active = 0 AND step IS NOT NULL ORDER BY activation_date DESC')
        return [dict(row) for row in rows]

    def get_active_users(self):
        rows = self._execute_query('SELECT * FROM users WHERE self_active = 1 AND admin_approved = 1 ORDER BY activation_date DESC')
        return [dict(row) for row in rows]

    def get_all_users(self):
        rows = self._execute_query('SELECT user_id, full_name, username, phone, self_active, created_at FROM users ORDER BY created_at DESC')
        return [dict(row) for row in rows]

    def get_selfbot_settings(self, user_id):
        rows = self._execute_query('SELECT * FROM selfbot_settings WHERE user_id = ?', (user_id,))
        if rows:
    ...
    def set_selfbot_settings(self, user_id, settings):
        ...
        self._execute_update(f'INSERT OR REPLACE INTO selfbot_settings ({columns}, updated_at) VALUES ({placeholders}, CURRENT_TIMESTAMP)', values)

    def update_selfbot_setting(self, user_id, key, value):
        self._execute_update(f'UPDATE selfbot_settings SET {key} = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?', (value, user_id))

    def get_buttons_enabled(self, user_id):
        rows = self._execute_query('SELECT buttons_enabled FROM selfbot_settings WHERE user_id = ?', (user_id,))
        return bool(rows[0][0]) if rows else True

    def set_buttons_enabled(self, user_id, enabled):
        self._execute_update('UPDATE selfbot_settings SET buttons_enabled = ? WHERE user_id = ?', (1 if enabled else 0, user_id))
    ...
    def update_ai_status(self, user_id, ai_status):
        for key, value in ai_status.items():
            if key.startswith('ai_'):
                self._execute_update(f'UPDATE selfbot_settings SET {key} = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?', (1 if value else 0, user_id))

    def add_enemy(self, owner_id, enemy_id, chat_type='pv'):
        self._execute_update('INSERT OR IGNORE INTO enemies (owner_id, enemy_id, chat_type) VALUES (?, ?, ?)', (owner_id, enemy_id, chat_type))
        return True

    def remove_enemy(self, owner_id, enemy_id, chat_type='pv'):
        self._execute_update('DELETE FROM enemies WHERE owner_id = ? AND enemy_id = ? AND chat_type = ?', (owner_id, enemy_id, chat_type))

    def get_enemies(self, owner_id, chat_type='pv'):
        rows = self._execute_query('SELECT enemy_id FROM enemies WHERE owner_id = ? AND chat_type = ?', (owner_id, chat_type))
    ...
    def add_locked_pv(self, owner_id, locked_user_id):
        self._execute_update('INSERT OR IGNORE INTO locked_pvs (owner_id, locked_user_id) VALUES (?, ?)', (owner_id, locked_user_id))

    def remove_locked_pv(self, owner_id, locked_user_id):
        self._execute_update('DELETE FROM locked_pvs WHERE owner_id = ? AND locked_user_id = ?', (owner_id, locked_user_id))

    def get_locked_pvs(self, owner_id):
        rows = self._execute_query('SELECT locked_user_id FROM locked_pvs WHERE owner_id = ?', (owner_id,))
        return [row[0] for row in rows]
    ...
    def set_reaction(self, owner_id, chat_id, target_id, emoji):
        self._execute_update('INSERT OR REPLACE INTO reactions (owner_id, chat_id, target_id, emoji) VALUES (?, ?, ?, ?)', (owner_id, chat_id, target_id, emoji))

    def get_reaction(self, owner_id, chat_id, target_id):
        rows = self._execute_query('SELECT emoji FROM reactions WHERE owner_id = ? AND chat_id = ? AND target_id = ?', (owner_id, chat_id, target_id))
        return rows[0][0] if rows else None

    def remove_reaction(self, owner_id, chat_id, target_id):
        self._execute_update('DELETE FROM reactions WHERE owner_id = ? AND chat_id = ? AND target_id = ?', (owner_id, chat_id, target_id))

    def add_filter_word(self, owner_id, word):
        self._execute_update('INSERT OR IGNORE INTO filter_words (owner_id, word) VALUES (?, ?)', (owner_id, word))

    def remove_filter_word(self, owner_id, word):
        self._execute_update('DELETE FROM filter_words WHERE owner_id = ? AND word = ?', (owner_id, word))
    ...
    def toggle_filter_word_by_id(self, owner_id, word_id):
        rows = self._execute_query('SELECT enabled FROM filter_words WHERE owner_id = ? AND id = ?', (owner_id, word_id))
        if not rows:
            return None
        new_state = 0 if rows[0][0] else 1
        self._execute_update('UPDATE filter_words SET enabled = ? WHERE owner_id = ? AND id = ?', (new_state, owner_id, word_id))
        return bool(new_state)

    def get_filter_words(self, owner_id):
        rows = self._execute_query('SELECT id, word, enabled FROM filter_words WHERE owner_id = ? ORDER BY id', (owner_id,))
        return [{'id': row[0], 'word': row[1], 'enabled': bool(row[2])} for row in rows]

    def get_filter_enabled(self, owner_id):
        rows = self._execute_query('SELECT filter_enabled FROM selfbot_settings WHERE user_id = ?', (owner_id,))
        return rows[0][0] if rows else 0

    def set_filter_enabled(self, owner_id, enabled):
        self._execute_update('UPDATE selfbot_settings SET filter_enabled = ? WHERE user_id = ?', (1 if enabled else 0, owner_id))

    def get_spam_settings(self, owner_id):
        rows = self._execute_query('SELECT * FROM spam_settings WHERE owner_id = ?', (owner_id,))
        if rows:
            return dict(rows[0])
    ...
    def set_spam_settings(self, owner_id, spam_protection=None, spam_limit=None, mute_duration=None):
        existing = self._execute_query('SELECT id FROM spam_settings WHERE owner_id = ?', (owner_id,))
        settings = {}
        ...
        if existing:
            ...
            self._execute_update(f'UPDATE spam_settings SET {set_clause} WHERE owner_id = ?', values)
        else:
            ...
            self._execute_update(f'INSERT INTO spam_settings ({columns}) VALUES ({placeholders})', values)

    def get_user_lock(self, owner_id, target_id, lock_type):
        rows = self._execute_query('SELECT enabled FROM user_locks WHERE owner_id = ? AND target_id = ? AND lock_type = ?', (owner_id, target_id, lock_type))
        return bool(rows[0][0]) if rows else False

    def set_user_lock(self, owner_id, target_id, lock_type, enabled):
        self._execute_update('INSERT OR REPLACE INTO user_locks (owner_id, target_id, lock_type, enabled) VALUES (?, ?, ?, ?)', (owner_id, target_id, lock_type, 1 if enabled else 0))

    def get_original_name(self, owner_id):
        rows = self._execute_query('SELECT value FROM user_info WHERE user_id = ? AND key = "original_name" ORDER BY timestamp DESC LIMIT 1', (owner_id,))
        return rows[0][0] if rows else None

    def set_original_name(self, owner_id, original_name):
        self._execute_update('INSERT INTO user_info (user_id, key, value) VALUES (?, "original_name", ?)', (owner_id, original_name))

    def get_current_name(self, owner_id):
        rows = self._execute_query('SELECT value FROM user_info WHERE user_id = ? AND key = "current_name" ORDER BY timestamp DESC LIMIT 1', (owner_id,))
        return rows[0][0] if rows else None

    def set_current_name(self, owner_id, current_name):
        self._execute_update('INSERT INTO user_info (user_id, key, value) VALUES (?, "current_name", ?)', (owner_id, current_name))

    def get_user_name(self, user_id):
        rows = self._execute_query('SELECT known_name, first_name, username FROM user_memory WHERE user_id = ?', (user_id,))
        if rows:
            known_name, first_name, username = rows[0]
            if known_name:
                return known_name
            elif first_name:
                return first_name
    ...
    def get_user_info(self, user_id, key=None):
        if key:
            rows = self._execute_query('SELECT value FROM user_info WHERE user_id = ? AND key = ? ORDER BY timestamp DESC LIMIT 1', (user_id, key))
            return rows[0][0] if rows else None
        else:
            rows = self._execute_query('SELECT key, value FROM user_info WHERE user_id = ?', (user_id,))
            return {row[0]: row[1] for row in rows} if rows else {}

    def update_user_memory(self, user_id, username, first_name, last_name, chat_id, known_name=None):
        existing = self._execute_query('SELECT user_id FROM user_memory WHERE user_id = ?', (user_id,))
        if existing:
            self._execute_update('UPDATE user_memory SET username = ?, first_name = ?, last_name = ?, known_name = ?, chat_id = ?, last_seen = CURRENT_TIMESTAMP WHERE user_id = ?', (username, first_name, last_name, known_name, chat_id, user_id))
        else:
            self._execute_update('INSERT INTO user_memory (user_id, username, first_name, last_name, known_name, chat_id, last_seen) VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)', (user_id, username, first_name, last_name, known_name, chat_id))

    def get_bio_setting(self, user_id, setting_name):
        rows = self._execute_query('SELECT status FROM bio_settings WHERE user_id = ? AND setting_name = ?', (user_id, setting_name))
        return rows[0][0] if rows else 'خاموش'

    def set_bio_setting(self, user_id, setting_name, status):
        self._execute_update('INSERT OR REPLACE INTO bio_settings (user_id, setting_name, status) VALUES (?, ?, ?)', (user_id, setting_name, status))

    def get_monshi_status(self, user_id):
        rows = self._execute_query('SELECT status, answer FROM monshi_status WHERE user_id = ?', (user_id,))
        if rows:
            return {'status': bool(rows[0][0]), 'answer': rows[0][1]}
        return {'status': False, 'answer': ''}

    def set_monshi_status(self, user_id, status, answer=''):
        self._execute_update('INSERT OR REPLACE INTO monshi_status (user_id, status, answer) VALUES (?, ?, ?)', (user_id, 1 if status else 0, answer))

    def add_answer(self, user_id, question, answer):
        self._execute_update('INSERT OR REPLACE INTO bot_answers (user_id, question, answer) VALUES (?, ?, ?)', (user_id, question, answer))

    def remove_answer(self, user_id, question):
        self._execute_update('DELETE FROM bot_answers WHERE user_id = ? AND question = ?', (user_id, question))

    def get_answers(self, user_id):
        rows = self._execute_query('SELECT question, answer FROM bot_answers WHERE user_id = ?', (user_id,))
        return {row[0]: row[1] for row in rows} if rows else {}

    def set_auto_comment(self, owner_id, channel_id, comment_text, channel_title, channel_type, channel_username):
        self._execute_update('INSERT OR REPLACE INTO auto_comments (owner_id, channel_id, comment_text, channel_title, channel_type, channel_username) VALUES (?, ?, ?, ?, ?, ?)', (owner_id, channel_id, comment_text, channel_title, channel_type, channel_username))

    def get_auto_comments(self, owner_id):
        rows = self._execute_query('SELECT * FROM auto_comments WHERE owner_id = ?', (owner_id,))
        return [dict(row) for row in rows]

    def get_auto_comment(self, owner_id, channel_id):
        rows = self._execute_query('SELECT * FROM auto_comments WHERE owner_id = ? AND channel_id = ?', (owner_id, channel_id))
        return dict(rows[0]) if rows else None

    def remove_auto_comment(self, owner_id, channel_id):
        self._execute_update('DELETE FROM auto_comments WHERE owner_id = ? AND channel_id = ?', (owner_id, channel_id))

    def mark_comment_sent(self, owner_id, channel_id, message_id):
        self._execute_update('INSERT OR REPLACE INTO sent_comments (owner_id, channel_id, message_id, comment_sent) VALUES (?, ?, ?, 1)', (owner_id, channel_id, message_id))

    def is_comment_sent(self, owner_id, channel_id, message_id):
        rows = self._execute_query('SELECT comment_sent FROM sent_comments WHERE owner_id = ? AND channel_id = ? AND message_id = ?', (owner_id, channel_id, message_id))
        return rows and rows[0][0] == 1

    def cache_message(self, owner_id, chat_id, message_id, message_text):
        self._execute_update('INSERT OR REPLACE INTO message_cache (owner_id, chat_id, message_id, message_text) VALUES (?, ?, ?, ?)', (owner_id, chat_id, message_id, message_text))

    def get_cached_message(self, owner_id, chat_id, message_id):
        rows = self._execute_query('SELECT message_text FROM message_cache WHERE owner_id = ? AND chat_id = ? AND message_id = ?', (owner_id, chat_id, message_id))
        return rows[0][0] if rows else None

    def add_enemy_spam_message(self, owner_id, spam_text):
        self._execute_update('INSERT INTO enemy_spam_messages (owner_id, spam_text) VALUES (?, ?)', (owner_id, spam_text))

    def get_enemy_spam_messages(self, owner_id):
        rows = self._execute_query('SELECT id, spam_text FROM enemy_spam_messages WHERE owner_id = ? ORDER BY created_at', (owner_id,))
        return [{'id': row[0], 'text': row[1]} for row in rows]

    def clear_enemy_spam_messages(self, owner_id):
        self._execute_update('DELETE FROM enemy_spam_messages WHERE owner_id = ?', (owner_id,))

    def delete_enemy_spam_message(self, owner_id, message_id):
        self._execute_update('DELETE FROM enemy_spam_messages WHERE owner_id = ? AND id = ?', (owner_id, message_id))

    def add_pinned_message(self, owner_id, chat_id, message_id):
        self._execute_update('INSERT OR IGNORE INTO pinned_messages (owner_id, chat_id, message_id) VALUES (?, ?, ?)', (owner_id, chat_id, message_id))

    def get_pinned_messages(self, owner_id, chat_id):
        rows = self._execute_query('SELECT message_id FROM pinned_messages WHERE owner_id = ? AND chat_id = ?', (owner_id, chat_id))
        return [row[0] for row in rows]
    ...
    def update_broadcast_stats(self, broadcast_id, sent_count, failed_count):
        self._execute_update('UPDATE broadcasts SET sent_count = ?, failed_count = ? WHERE id = ?', (sent_count, failed_count, broadcast_id))

    def save_bio(self, user_id, bio_text):
        self._execute_update('INSERT INTO user_bio (user_id, bio_text) VALUES (?, ?)', (user_id, bio_text))

    def get_bio(self, user_id):
        rows = self._execute_query('SELECT bio_text FROM user_bio WHERE user_id = ? ORDER BY created_at DESC LIMIT 1', (user_id,))
...
class SelfBotManager:
        ...
        try:
            if os.path.exists(self.STATE_FILE):
                with open(self.STATE_FILE, 'r', encoding='utf-8') as f:
                    ...
                    self.time_font_indices = data.get('time_font_indices', 'all')
    ...
    def save_state(self):
        try:
            data = {
                ...
                'time_font_indices': self.time_font_indices
            }
    ...
    async def get_price_info(self, query):
        ...
        if query_lower in ['دلار', 'dollar', 'usd']:
            ...
            return "❌ ارز مورد نظر یافت نشد یا خطا در دریافت اطلاعات"
    ...
    async def handle_new_message(self, event):
        ...
        is_group = isinstance(chat, (types.Chat, types.Channel))
        ...
        if is_private:
            ...
            if sender_id in enemies:
                ...
                if not spam_messages:
                    spam_messages = [{"text": m} for m in SPAM_MESSAGES]
                for spam_msg in spam_messages:
                    try:
                        await self.client.send_message(sender_id, spam_msg['text'])
        ...
        settings = db.get_selfbot_settings(self.user_id)
        if is_private:
            active_ai = self.get_active_ai('pm')
            ...
            active_ai = self.get_active_ai('group')
            if active_ai and event.is_reply and event.text:
        ...
        # 3. فیلتر کلمات (اگر فعال باشد)
        if db.get_filter_enabled(self.user_id):
            filters = db.get_filter_words(self.user_id)
            for word_info in filters:
                if word_info['enabled'] and word_info['word'] in event.text:
                    await event.delete()
                    await self.client.send_message(chat_id, f"⚠️ پیام شما به دلیل کلمه فیلتر شده '{word_info['word']}' حذف شد")
                    return

        # 4. قفل پیوی (اگر فعال باشد)
        if is_private and db.is_pv_locked(self.user_id, sender_id):
            await event.delete()
            return

        # 5. قفل کلی پیوی
        if is_private and settings.get('pv_lock_all', False):
            await event.delete()
            return
    ...
    async def auto_comment_handler(self, event):
        ...
        if hasattr(chat, 'broadcast') and chat.broadcast:
            ...
            if auto_comment:
                if not db.is_comment_sent(self.user_id, channel_id, event.message.id):
                    await self.client.send_message(channel_id, auto_comment['comment_text'])
            ...
            try:
                if font_arg.isdigit():
                    ...
                    if 0 <= idx < len(classic_fonts):
                        ...
                        elif isinstance(self.time_font_indices, list):
                            if idx not in self.time_font_indices:
                                self.time_font_indices.append(idx)
                        else:
                            self.time_font_indices = [idx]
                        self.save_state()
                        db.update_selfbot_setting(self.user_id, 'time_font_indices', ','.join(map(str, self.time_font_indices)) if isinstance(self.time_font_indices, list) else 'all')
                        await event.edit(f"✅ فونت شماره {idx} به لیست اضافه شد")
                    else:
                    ...
                    self.time_font_indices = []
                    self.save_state()
                    db.update_selfbot_setting(self.user_id, 'time_font_indices', '')
...
def get_user_api(user_id):
    conn = sqlite3.connect('main_database.db', timeout=30, check_same_thread=False)
    cursor = conn.cursor()
...
def get_main_panel_keyboard(user_id):
    ...
    style_primary = "primary" if buttons_enabled else "secondary"
    style_success = "success" if buttons_enabled else "secondary"
    style_danger = "danger" if buttons_enabled else "secondary"
...
def get_font_menu_keyboard(user_id):
    ...
    if manager:
        selected_indices = manager.time_font_indices
    ...
    keyboard.append([
        InlineKeyboardButton("🎨 همه", callback_data=f"exec_font_all_{user_id}"),
        InlineKeyboardButton("🗑️ پاک", callback_data=f"exec_font_clear_{user_id}")
    ])
    keyboard.append([
        InlineKeyboardButton("✅ ذخیره", callback_data=f"exec_font_save_{user_id}")
    ])
...
def get_buttons_menu_keyboard(user_id):
    buttons_enabled = db.get_buttons_enabled(user_id)
    style_success = "success" if buttons_enabled else "secondary"
    style_danger = "danger" if not buttons_enabled else "secondary"
    style_primary = "primary" if buttons_enabled else "secondary"
...
async def restore_backup(file_path):
    try:
        ...
        try:
            conn = sqlite3.connect(os.path.join(extract_folder, 'main_database.db'), timeout=30, check_same_thread=False)
            cursor = conn.cursor()
...
async def scheduled_backup_task(app):
    while True:
...
# ======================================================
# توابع فراخوانی API و مدیریت پیام‌ها
# ======================================================
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("font_menu_"):
        await query.edit_message_text("🎨 **انتخاب فونت تایم**\n\nفونت‌های مورد نظر را انتخاب کنید (چندگانه):", reply_markup=get_font_menu_keyboard(user_id))
        return
...
# ======================================================
# تابع exec_command_handler (اصلاح‌شده)
# ======================================================
async def exec_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == 'time_on':
        ...
        try:
            await query.edit_message_text(query.message.text, reply_markup=get_time_menu_keyboard(user_id))
        except Exception:
            await context.bot.send_message(chat_id=chat_id, text="✅ تایم روشن شد", reply_markup=get_time_menu_keyboard(user_id))
        return
    ...
    if cmd == 'time_flag':
        ...
        try:
            ...
            await context.bot.send_message(chat_id=chat_id, text="✅ تایمر پرچم روشن شد", reply_markup=get_time_menu_keyboard(user_id))
        return
    ...
    if cmd == 'time_off':
        ...
        try:
            ...
            await context.bot.send_message(chat_id=chat_id, text="✅ تایم خاموش شد", reply_markup=get_time_menu_keyboard(user_id))
        return

    # ========== دستورات فونت ==========
    if cmd.startswith('font_') and cmd != 'font_all' and cmd != 'font_clear' and cmd != 'font_save':
        try:
            font_index = int(cmd.split('_')[1])
            if 0 <= font_index < len(classic_fonts):
                if manager.time_font_indices == 'all':
                    manager.time_font_indices = []
                if isinstance(manager.time_font_indices, list):
                    if font_index in manager.time_font_indices:
                        manager.time_font_indices.remove(font_index)
                    else:
                        manager.time_font_indices.append(font_index)
                else:
                    manager.time_font_indices = [font_index]
                await msg.edit_text(f"✅ فونت {classic_fonts[font_index]} {'اضافه شد' if font_index in manager.time_font_indices else 'حذف شد'}")
                try:
                    await query.edit_message_text(query.message.text, reply_markup=get_font_menu_keyboard(user_id))
                except Exception:
                    await context.bot.send_message(chat_id=chat_id, text="✅ فونت به‌روزرسانی شد", reply_markup=get_font_menu_keyboard(user_id))
            else:
    ...
    if cmd == 'font_all':
        ...
        try:
            ...
            await context.bot.send_message(chat_id=chat_id, text="✅ همه فونت‌ها فعال شدند", reply_markup=get_font_menu_keyboard(user_id))
        return
    ...
    if cmd == 'font_clear':
        ...
        try:
            ...
            await context.bot.send_message(chat_id=chat_id, text="✅ لیست فونت‌ها پاک شد", reply_markup=get_font_menu_keyboard(user_id))
        return

    if cmd == 'font_save':
        manager.save_state()
        db.update_selfbot_setting(user_id, 'time_font_indices', ','.join(map(str, manager.time_font_indices)) if isinstance(manager.time_font_indices, list) else 'all')
        await msg.edit_text("✅ تنظیمات فونت ذخیره شد")
        try:
            await query.edit_message_text(query.message.text, reply_markup=get_font_menu_keyboard(user_id))
        except Exception:
            await context.bot.send_message(chat_id=chat_id, text="✅ تنظیمات فونت ذخیره شد", reply_markup=get_font_menu_keyboard(user_id))
        return
    ...
    if cmd.startswith('flag_'):
        ...
        if flag in flags:
            ...
            try:
                await query.edit_message_text(query.message.text, reply_markup=get_flagselect_menu_keyboard(user_id))
            except Exception:
                await context.bot.send_message(chat_id=chat_id, text=f"✅ پرچم {flag} انتخاب شد", reply_markup=get_flagselect_menu_keyboard(user_id))
        else:
    ...
    if cmd.startswith('ai_pm_'):
        ...
        try:
            await query.edit_message_text(query.message.text, reply_markup=get_ai_menu_keyboard(user_id))
        except Exception:
            await context.bot.send_message(chat_id=chat_id, text="✅ هوش مصنوعی به‌روزرسانی شد", reply_markup=get_ai_menu_keyboard(user_id))
        return
    ...
    if cmd == 'buttons_on':
        ...
        try:
            await query.edit_message_text(query.message.text, reply_markup=get_buttons_menu_keyboard(user_id))
        except Exception:
            await context.bot.send_message(chat_id=chat_id, text="✅ دکمه‌ها رنگی شدند", reply_markup=get_buttons_menu_keyboard(user_id))
        return
    ...
    if cmd == 'buttons_off':
        ...
        try:
            ...
            await context.bot.send_message(chat_id=chat_id, text="✅ دکمه‌ها سفید شدند", reply_markup=get_buttons_menu_keyboard(user_id))
        return
...
🎨 **فونت تایم**: با کلیک روی این دکمه، می‌توانید فونت ساعت را انتخاب کنید (چندگانه)
...
✅ برای انتخاب چندگانه، روی هر فونت کلیک کنید تا تیک بخورد یا برداشته شود.
🎨 **همه**: با انتخاب این گزینه، همه فونت‌ها به صورت چرخشی تغییر می‌کنند.
🗑️ **پاک**: لیست فونت‌های انتخاب شده را پاک می‌کند.
✅ **ذخیره**: تغییرات را ذخیره می‌کند.
""",
        'help_flag': """
...
"""
    ...
    if help_key in help_texts:
        await query.edit_message_text(help_texts[help_key], reply_markup=InlineKeyboardMarkup([
...
async def main():
    ...
    app.add_handler(CommandHandler("cancel", lambda u, c: handle_cancel(u, c)))
    ...
    await app.start()
    await app.updater.start_polling(allowed_updates=Update.ALL_TYPES, timeout=30, drop_pending_updates=True)
    ...
    asyncio.create_task(scheduled_backup_task(app))
