...
# ======================================================
# تنظیمات هوش مصنوعی جدید (۵ سرویس)
# ======================================================
AI_CONFIGS = {
    'deepseek': {
    ...
    'openai': {
        'url': 'https://ai.aimlapi.com/v1/chat/completions',
...
class MainDatabase:
    ...
    def init_database(self):
        ...
        # ستون‌های جدید هوش مصنوعی در selfbot_settings
        cursor.execute("PRAGMA table_info(selfbot_settings)")
        columns = [col[1] for col in cursor.fetchall()]
        new_ai_columns = ['ai_4_pm', 'ai_5_pm', 'ai_4_group', 'ai_5_group']
        for col in new_ai_columns:
            if col not in columns:
                cursor.execute(f"ALTER TABLE selfbot_settings ADD COLUMN {col} BOOLEAN DEFAULT 0")
        
        conn.commit()
        conn.close()
        logger.info("✓ دیتابیس اصلی ایجاد شد (و ستون‌های جدید هوش اضافه شدند)")
    ...
    def get_selfbot_settings(self, user_id):
        ...
        if row:
            ...
            # ai_status برای ۵ هوش
            settings['ai_status'] = {
                f'ai_{i}_pm': bool(settings.get(f'ai_{i}_pm', 0)) for i in range(1, 6)
            }
            settings['ai_status'].update({
                f'ai_{i}_group': bool(settings.get(f'ai_{i}_group', 0)) for i in range(1, 6)
            })
            settings['translate'] = {
            ...
            default_settings = {
                ...
                'selfbot_enabled': 1,
                'ai_status': {
                    f'ai_{i}_pm': False for i in range(1, 6)
                },
                'ai_status': {
                    **{f'ai_{i}_pm': False for i in range(1, 6)},
                    **{f'ai_{i}_group': False for i in range(1, 6)}
                },
                'translate': {
                    'english': False,
                    'arabic': False,
    ...
    def update_ai_status(self, user_id, ai_status):
        ...
        for key, value in ai_status.items():
            if key in [f'ai_{i}_pm' for i in range(1, 6)] + [f'ai_{i}_group' for i in range(1, 6)]:
...
# ======================================================
# تابع جدید هوش مصنوعی با ۵ سرویس
# ======================================================
async def get_ai_response(text, ai_type, user_id=None):
    """
    ai_type: 1=deepseek, 2=chatgpt, 3=grok, 4=blackbox, 5=openai
    """
    try:
        if ai_type == 1:
            config = AI_CONFIGS['deepseek']
        elif ai_type == 2:
            config = AI_CONFIGS['chatgpt']
        elif ai_type == 3:
            config = AI_CONFIGS['grok']
        elif ai_type == 4:
            config = AI_CONFIGS['blackbox']
        elif ai_type == 5:
...
⏱️ مدت زمان: {spam_settings.get('mute_duration', 10)} ثانیه
        ...
        # ========== هوش مصنوعی (۵ سرویس) ==========
        ai_names = ['', 'دیپ سیک', 'چت جی‌پی‌تی', 'گراک', 'بلک‌باکس', 'OpenAI']
        if cmd == 'پیوی' and args and args[0] in ['۱', '۲', '۳', '۴', '۵'] and len(args) == 1:
            ai_status = settings.get('ai_status', {})
            ai_num = int(args[0])
            # reset all pm
            for i in range(1, 6):
                ai_status[f'ai_{i}_pm'] = False
            ai_status[f'ai_{ai_num}_pm'] = True
            db.update_ai_status(self.user_id, ai_status)
            await event.edit(f'✅ هوش {ai_num} ({ai_names[ai_num]}) در پی‌وی روشن شد')
            return
        ...
        if cmd == 'گروه' and args and args[0] in ['۱', '۲', '۳', '۴', '۵'] and len(args) == 1:
            ai_status = settings.get('ai_status', {})
            ai_num = int(args[0])
            for i in range(1, 6):
                ai_status[f'ai_{i}_group'] = False
            ai_status[f'ai_{ai_num}_group'] = True
            db.update_ai_status(self.user_id, ai_status)
            await event.edit(f'✅ هوش {ai_num} ({ai_names[ai_num]}) در گروه روشن شد')
            return
...
ꕀꔚꨄꕣꕥ✺ღდ
    ...
    def format_status_info(self, settings):
        ...
        for i in range(1, 6):
            if ai_status.get(f'ai_{i}_pm'):
                active_ai_pm = f"هوش {i} ({ai_names[i]})"
                break
        active_ai_group = "هیچ هوش فعالی در گروه وجود ندارد"
        for i in range(1, 6):
            if ai_status.get(f'ai_{i}_group'):
                active_ai_group = f"هوش {i} ({ai_names[i]})"
                break
...
ꕀꔚꨄꕣꕥ✺ღდ
    ...
    async def handle_new_message(self, event):
        ...
        # ========== ریکشن خودکار - در گروه/کانال هم کار می‌کند ==========
                ...
                if reaction and reaction in ALLOWED_EMOJIS:
                    try:
                        ...
                        logger.warning(
                            ...
                            f"(ممکن است موقتی باشد، قانون حذف نشد و دفعه بعد دوباره تلاش می‌شود)."
                        )
                    except Exception as e:
        ...
        # ========== هوش مصنوعی (۵ سرویس) در گروه ==========
        if not event.message.out and event.message.text and isinstance(event.message.peer_id, (PeerChannel, PeerChat)):
            # بررسی اگر پیام به ما منشن شده باشد
            if event.message.mentioned or (event.message.text and f'@{self.BASE_NAME}' in event.message.text):
                ai_status = settings.get('ai_status', {})
                ai_type = None
                for i in range(1, 6):
                    if ai_status.get(f'ai_{i}_group'):
                ...
                if ai_type:
                    try:
                        ...
                        logger.error(f"خطا در پاسخ هوش مصنوعی در گروه: {e}")
...
# ======================================================
# منوی هوش مصنوعی با ۵ سرویس
# ======================================================
def get_ai_menu_keyboard(user_id):
    settings = db.get_selfbot_settings(user_id)
    ai = settings['ai_status']
    keyboard = []
    # ردیف اول: پیوی ۱ تا ۳
    row1 = []
    for i in range(1, 4):
        status = ai.get(f'ai_{i}_pm', False)
        label = f"🟢 پیوی {i} {'' if not status else '✓'}"
        callback = f"exec_ai_pm_{i}_{user_id}"
        style = "success" if not status else "primary"
        row1.append(InlineKeyboardButton(label, callback_data=callback, style=style))
    ...
    for i in range(4, 6):
        ...
        callback = f"exec_ai_pm_{i}_{user_id}"
        style = "success" if not status else "primary"
        row2.append(InlineKeyboardButton(label, callback_data=callback, style=style))
    keyboard.append(row2)
    # خاموش پیوی
    keyboard.append([InlineKeyboardButton("⚫ خاموش پیوی", callback_data=f"exec_ai_pm_off_{user_id}", style="danger")])
    # ردیف سوم: گروه ۱ تا ۳
    row3 = []
    for i in range(1, 4):
        status = ai.get(f'ai_{i}_group', False)
        label = f"🟢 گروه {i} {'' if not status else '✓'}"
        callback = f"exec_ai_group_{i}_{user_id}"
        style = "success" if not status else "primary"
        row3.append(InlineKeyboardButton(label, callback_data=callback, style=style))
    keyboard.append(row3)
    # ردیف چهارم: گروه ۴ و ۵
    row4 = []
    for i in range(4, 6):
        status = ai.get(f'ai_{i}_group', False)
        label = f"🟢 گروه {i} {'' if not status else '✓'}"
        callback = f"exec_ai_group_{i}_{user_id}"
        style = "success" if not status else "primary"
        row4.append(InlineKeyboardButton(label, callback_data=callback, style=style))
    keyboard.append(row4)
    # خاموش گروه
    keyboard.append([InlineKeyboardButton("⚫ خاموش گروه", callback_data=f"exec_ai_group_off_{user_id}", style="danger")])
    # بازگشت
...
async def exec_command_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == 'monshi_on':
        ...
        try:
            await query.message.edit_text(query.message.text, reply_markup=get_monshi_menu_keyboard(user_id))
        except Exception as _panel_refresh_err:
            print(f"⚠️ [DEBUG پنل] رفرش دکمه‌های پنل قدیمی fail شد (احتمالاً پیام قدیمی/غیرقابل‌دسترسه، مشکلی نیست چون خود عملیات انجام شده): {type(_panel_refresh_err).__name__}: {_panel_refresh_err}")
        return
    if cmd == 'monshi_off':
...
ꕀꔚꨄꕣꕥ✺ღდ
    ...
    if cmd == 'filter_on':
        ...
        try:
            await query.message.edit_text(query.message.text, reply_markup=get_filter_menu_keyboard(user_id))
        except Exception as _panel_refresh_err:
            print(f"⚠️ [DEBUG پنل] رفرش دکمه‌های پنل قدیمی fail شد (احتمالاً پیام قدیمی/غیرقابل‌دسترسه، مشکلی نیست چون خود عملیات انجام شده): {type(_panel_refresh_err).__name__}: {_panel_refresh_err}")
        return
    if cmd == 'filter_off':
...
"""
    ...
    # ======================================================
    # هوش مصنوعی (۵ سرویس) - پردازش دکمه‌ها
    # ======================================================
    ai_names = ['', 'دیپ سیک', 'چت جی‌پی‌تی', 'گراک', 'بلک‌باکس', 'OpenAI']
    # پیوی
    if cmd.startswith('ai_pm_'):
        try:
            num = int(cmd.split('_')[2])  # ai_pm_1 -> num=1
            if 1 <= num <= 5:
                ai_status = {}
                for i in range(1, 6):
                    ai_status[f'ai_{i}_pm'] = (i == num)
                db.update_ai_status(user_id, ai_status)
                await msg.edit_text(f"✅ هوش {num} ({ai_names[num]}) در پی‌وی روشن شد")
                try:
    ...
    if cmd.startswith('ai_group_'):
        try:
            num = int(cmd.split('_')[2])  # ai_group_1 -> num=1
            if 1 <= num <= 5:
                ai_status = {}
                for i in range(1, 6):
                    ai_status[f'ai_{i}_group'] = (i == num)
                db.update_ai_status(user_id, ai_status)
                await msg.edit_text(f"✅ هوش {num} ({ai_names[num]}) در گروه روشن شد")
                try:
                    await query.message.edit_text(query.message.text, reply_markup=get_ai_menu_keyboard(user_id))
                except Exception as _panel_refresh_err:
                    print(f"⚠️ [DEBUG پنل] رفرش دکمه‌های پنل قدیمی fail شد (احتمالاً پیام قدیمی/غیرقابل‌دسترسه، مشکلی نیست چون خود عملیات انجام شده): {type(_panel_refresh_err).__name__}: {_panel_refresh_err}")
    ...
    if cmd == 'autosend_on':
        ...
        try:
            await query.message.edit_text(query.message.text, reply_markup=get_message_menu_keyboard(user_id))
        except Exception as _panel_refresh_err:
            print(f"⚠️ [DEBUG پنل] رفرش دکمه‌های پنل قدیمی fail شد (احتمالاً پیام قدیمی/غیرقابل‌دسترسه، مشکلی نیست چون خود عملیات انجام شده): {type(_panel_refresh_err).__name__}: {_panel_refresh_err}")
        return
    if cmd == 'autosend_off':
...
async def main():
    print("=" * 60)
    print("🤖 سیستم جامع عضویت و سلف‌بات v4.8.0 (پچ هوش ۵ سرویس)")
...
if __name__ == '__main__':
    print("=" * 60)
    print("🔧 نسخه اصلاح‌شده - PATCH: ۵ هوش جدید (DeepSeek, ChatGPT, Grok, Blackbox, OpenAI)")
    print("=" * 60)
    logger.info("🔧 نسخه اصلاح‌شده در حال اجراست - PATCH-2026-07-01-v3")
